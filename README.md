...
失败原因按确定性排序，而不是按「哪一步先失败」：relogin 站点在登录前必然读不到额度，因此「读不到额度」是最弱的线索，不会盖住 WAF 拦截、停在第三方登录页、未拿到 state 这些确定原因；登录前那次 `/api/user/self` 的 401 也不再被当成「站点原始错误」附到结论上。停在第三方登录页时会额外记录认证 Cookie 是否真的进了浏览器上下文：在 → 会话已过期，重新捕获登录态；不在 → 登录态没装载成功，重新捕获也不会变好。
...
**挑战页与拦截页是两种失败**，工具会分开处理：

| 响应 | 特征 | 处理 |
| --- | --- | --- |
| 挑战页 | `Just a moment` / `cf_chl_opt`，阿里云 WAF 的混淆 JS | 可解，按需启动一次浏览器执行 JS 并导出 Cookie |
| 拦截页 | HTTP 403 + `Sorry, you have been blocked`（error 1020 类） | 出口 IP 被站点安全规则封禁，浏览器同样过不去；不启动浏览器，结论直接给出 Ray ID 与站点回显的出口 IP，请更换代理节点或联系站长放行 |

站点回 HTML 而不是 JSON 时（拦截页、登录页、502 页），结论只保留一行归纳（页面标题 + 首个标题行），不会把整页 HTML 灌进汇总与结果文件。
